...
class RendimentosController extends Controller
{
            ...
            foreach ($investimentosRendimentos as $IR) {
                ...
                foreach ($IR->rendimentos as $rendimento) {
                    $json = [
                        ...
                        "categoria_investimentos" => $IR->categoria_investimentos->categoria_investimentos,
    ...
    public function store(Request $request)
    {
        try {
            Rendimentos::create([
                ...
                'valor_rendimento' => str_replace(',', '.', $request->valor_rendimento),
    ...
    {
        try {
            Rendimentos::where('id', $id)->delete();

            return response()->json([
                'status' => true,
                'mensagem' => 'Rendimento Removido Com Sucesso!'
            ], 200);
        } catch (Exception $e) {
            return response()->json([
                'status' => false,
